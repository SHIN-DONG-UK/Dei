...
DEVICE_NORMALIZE = {
    "air_conditioner": ["aircon", "ac", "airconditioner", "air_conditioner"],
    "tv": ["tv", "TV", "television"],
}

ACTION_NORMALIZE = {
    "power_on": ["power", "on", "power_on", "turn_on", "turnon", "poweron"],
    "power_off": ["off", "power_off", "turn_off", "turnoff", "poweroff"],
    "set_temperature": ["set_temp", "settemp", "temperature", "set_temperature", "settemperature", "temp"],
}

CONTROL_TABLE = {
    "air_conditioner": {
        "set_temperature": set_aircon_temp,
        "power_on": aircon_on,
...
class ControlExectueNode(Node):
    ...
    def _normalize_command(self, raw_action: str) -> str:
    ...
    def _normalize_device(self, raw_device: str) -> str:
        raw = raw_device.lower().replace(" ", "").replace("-", "_")
        print(raw)
